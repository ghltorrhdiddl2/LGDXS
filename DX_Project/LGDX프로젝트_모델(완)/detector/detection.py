...
VIDEO_PATH = "videos/dog_nod.mp4"  # 비디오 파일 경로
...
FRAME_SKIP         = 3    # 5프레임당 1프레임 처리
WINDOW_SIZE        = 5    # 이상행동 감지를 위한 윈도우 크기 (5프레임)
DETECTION_WINDOWS  = 3    # 필요한 감지 횟수 (5번)
ALLOWED_NORMAL     = 3    # 허용되는 정상 행동 횟수
FPS               = 30.0  # 기본 FPS
BUFFER_SECONDS    = 6     # 저장할 영상 길이 (초)  #####################
MAX_FRAMES        = int(FPS * BUFFER_SECONDS)  # 버퍼에 저장할 최대 프레임 수
STD_MULTIPLIER    = 0.2   # 표준편차 배수 (민감도)
                    ...
                    if is_recording:
                        ...
                        if current_duration >= BUFFER_SECONDS:
                            ...
                            if current_stage > 0:
                                ...
                                print(f"\n✨ 정상 행동으로 판단됨 (0단계) - 영상은 저장하지 않고 DB에만 기록")
                                # 0단계일 때는 영상 없이 DB에만 저장
                                result = post_event(datetime.now().isoformat(),
                                                  current_stage,
                                                  current_summary,
                                                  None,  # 영상 데이터 없음
